...
class PreparedQuery[T](AbstractQuery[T]):
    ...
    def set_parameter(self, position: str | int, param: Any) -> None:
        ...
        try:
            index = self.input_signature_names.index(position) if isinstance(position, str) else position
            expected_type = self.input_signature_types[index]
        except Exception:
            raise PreparedQueryException("Invalid parameter name or position.")

        if self.language == "sql":
            if not PreparedQuery._TYPE_MAP[expected_type](param):  # Assuming type_map exists
                raise PreparedQueryException(f"Expected type {expected_type}, got {type(param).__name__}")
...
class PolledQuery[T](ABC, AbstractQuery[T]):
    ...
    def __init__(
        ...
        registration = self.client.json_rpc.call('create-polled-query', self.base_parameters)
        self.handle = registration['handle']
        self.signature = []
